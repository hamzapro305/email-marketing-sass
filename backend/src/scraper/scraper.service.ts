...
export class ScraperService {
   ...
   */
  async scrapeSite(website: string, domain: string): Promise<SiteScrapeResult> {
    const pages: PageContent[] = [];
    let error: string | null = null;

    const home = await this.fetchPage(website, domain);
    if (home) {
      pages.push(home);
      const candidates = this.discoverKeyPages(home, website);
      for (const url of candidates.slice(0, this.cfg.maxPagesPerSite - 1)) {
        const page = await this.fetchPage(url, domain);
  ...
  async fetchPage(url: string, domain: string): Promise<PageContent | null> {
    ...
    if (cached) {
      return cached.status === 'ok' ? this.toContent(cached) : null;
    }
  ...
  computeSignals(pages: PageContent[]): WebsiteSignals {
    ...
    return {
      ...
      hasBlog: matches(/blog|news|insights|articles/i),
